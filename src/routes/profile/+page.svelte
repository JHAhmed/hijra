				...
				<div class="lg:col-span-1">
					<div
						...
						<div class="flex flex-col items-center text-center">
							...
							<h2 class="text-xl font-bold tracking-tight text-secondary">{auth.user.name}</h2>
							<p class="mb-6 text-sm text-gray-500">{auth.user.email}</p>
							...
							<div class="mb-8 flex flex-wrap justify-center gap-2">
								<span
									...
									<Icon
										icon={auth.user.emailVerification
											? 'heroicons:check-circle-solid'
											: 'heroicons:exclamation-circle-solid'}
										class="h-3.5 w-3.5" />
									{auth.user.emailVerification ? 'Verified Email' : 'Email Unverified'}
								</span>
								...
								<span
									...
									{auth.user.status ? 'Active Account' : 'Inactive'}
								</span>
							</div>
					...
					<div class="mt-6 rounded-3xl border border-gray-200 bg-gray-50 p-6">
						...
						<div class="space-y-4">
							<div class="flex items-center justify-between">
								...
								<span class={`text-xs font-bold ${auth.user.mfa ? 'text-green-600' : 'text-gray-400'}`}>
									{auth.user.mfa ? 'ENABLED' : 'DISABLED'}
								</span>
							</div>
					...
					<div class="rounded-3xl border border-gray-200 bg-white p-6 md:p-8">
						...
						<div class="grid grid-cols-1 gap-x-12 gap-y-6 md:grid-cols-2">
							<div class="group border-b border-gray-100 pb-2">
								...
								<span class="block text-base font-medium text-secondary">{auth.user.name}</span>
							</div>

							<div class="group border-b border-gray-100 pb-2">
								<span class="mb-1 block text-xs font-medium text-gray-400">Email Address</span>
								<div class="flex items-center justify-between">
									<span class="block text-base font-medium text-secondary">{auth.user.email}</span>
									{#if !auth.user.emailVerification}
							...
							<div class="group border-b border-gray-100 pb-2">
								...
								{#if auth.user.phone}
									<span class="block text-base font-medium text-secondary">{auth.user.phone}</span>
								{:else}
									<button
							...
							<div class="group border-b border-gray-100 pb-2">
								...
								<div class="flex items-center gap-2">
									<span class="block font-mono text-base text-gray-500">{auth.user.$id}</span>
							...
							<div class="flex items-start gap-4">
								...
								<div>
									...
									<p class="text-sm text-gray-500">
										...
										<span class="font-medium text-gray-700">{formatTime(auth.user.accessedAt)}</span> on
										<span class="font-medium text-gray-700">{formatDate(auth.user.accessedAt)}</span>.
									</p>
								</div>
							</div>
								...
								<div>
									...
									<p class="text-sm text-gray-500">
										Account created on <span class="font-medium text-gray-700"
											>{formatDate(auth.user.registration)}</span
										>.
									</p>
								</div>
							</div>
